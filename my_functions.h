...
{
    if (sig == SIGUSR1)
        printf("A fájlon keresztüli küldés szolgáltatás nem elérhető.\n");

    if (sig == SIGINT)
    {
        printf("Viszlát! A program leáll...\n");
        exit(0);
    }

    if (sig == SIGALRM)
    {
        printf("Időtúllépés...A szerver nem válaszol!\n");
        exit(3);
    }
}
...
//    bytes = sendto(s, buffer, strlen(buffer)+1, flag, (struct sockaddr *) &server, server_size);
   bytes = sendto(s, &NumValues, sizeof(int), 0, (struct sockaddr *)&server, server_size);
   if ( bytes <= 0 ) {
   ...
   printf(" Server's (%s:%d) acknowledgement:\n  %s\n",
    ...
    bytes = sendto(s, values, sizeof(int) * NumValues, 0, (struct sockaddr *)&server, server_size);
    // bytes = sendto(s, values, sizeof(int), 0, (struct sockaddr *)&server, server_size);
    if (bytes < 0)
    {
    ...
    int n = recvfrom(s, buffer, sizeof(buffer), 0, NULL, NULL);
    if (n < 0)
    {
...
void ReceiveViaSocket(){
   ...
   setsockopt(s1, SOL_SOCKET, SO_KEEPALIVE, &on, sizeof on);

   /************************ Binding socket ********************/
   err = bind(s1, (struct sockaddr *) &server, server_size);
   if ( err < 0 ) {
   ...
   while(1){ // Continuous server operation
      ...
      bytes = recvfrom(s1, buffer, BUFSIZE, flag, (struct sockaddr *) &client, &client_size );
      if ( bytes < 0 ) {
        ...
        {                                                  // received first packet with an int
            ...
            sendto(s1, buffer, sizeof(int), 0, (struct sockaddr *)&client, client_size); // send back the int
        }
        else
        {                                                                    // received second packet with an array of ints
            memcpy(&numInts, buffer, sizeof(int));                       // extract the number of ints
            int *values = (int *)malloc(numInts * sizeof(int));              // allocate memory for the values
            memcpy(values, buffer + sizeof(int), numInts * sizeof(int)); // extract the values from the buffer
            printf("Received %d ints\n", numInts);

            // call BMPcreator function with values array here
            printf("IDE\n");
            BMPcreator(values, numInts);

            free(values);                                                                        // free the allocated memory
            sendto(s1, &numInts, sizeof(int), 0, (struct sockaddr *)&client, client_size); // send back the number of ints received
        }
    }
    ...
    //   bytes = sendto(s1, buffer, strlen(buffer)+1, flag, (struct sockaddr *) &client, client_size);
    //   if ( bytes <= 0 ) {
...
void SendViaSocket2(int *Values, int NumValues)
{
